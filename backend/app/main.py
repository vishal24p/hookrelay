...
def validate_session_id(session_id: str) -> bool:
    if not isinstance(session_id, str) or not 1 <= len(session_id) <= 100:
        return False
    return all(char.isalnum() or char in {"-", "_"} for char in session_id)


def is_safe_forward_url(forward_url: Any) -> tuple[bool, str | None]:
    ...
    if host in BLOCKED_FORWARD_HOSTS:
        return False, "forward_url host is not allowed"
    ...
    if address_is_blocked_for_forward(address) or not address.is_global:
